...
<section class="films best-practices">
    <div class="container">
        ...
        <div>
            <h3>  تحتوي قاعدة بيانات شبكة الأسماك على<span class="number">{{ $companies->total() }}</span> من
                @if(requestUri() == 'factories')
                    مصنعي الاسماك
                @elseif(requestUri() == 'sellers')
                    موزعي الاسماك
                @else
                    موردي المستلزمات
                @endif ، انضم الآن لشبكة الأسماك وسجل شركتك</h3>
            <h4>يمكنك الإنضمام إلى الشركات على قاعدة بيانات شبكة الأسماك
                <a href="{{ route('farmers.create') }}">من هنا</a></h4>
            <h4>
